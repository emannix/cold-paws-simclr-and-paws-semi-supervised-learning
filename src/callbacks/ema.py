...
class EMA(pl.Callback):
    ...
    def on_train_start(self, trainer: "pl.Trainer", pl_module: pl.LightningModule) -> None:
        ...
        if not self._ema_state_dict_ready and pl_module.global_rank == 0:
            ...
            self.param_keys = [k for k, _ in pl_module.named_parameters()]
            self.buffer_keys = [k for k, _ in pl_module.named_buffers()]
            self.ema_has_module = hasattr(pl_module, 'module')
    ...
    def on_train_batch_end(self, trainer: "pl.Trainer", pl_module: pl.LightningModule, *args, **kwargs) -> None:
        # Update EMA weights
        with torch.no_grad():
            needs_module = hasattr(pl_module, 'module') and not self.ema_has_module
            msd = self.get_state_dict(pl_module)
            esd = self.ema_state_dict
